...
* [x] **Asset Config 탭 추가**: 상단 헤더에 3번째 탭으로 추가.
* [x] **통합 입력 폼**: 환율 자동 계산 및 통화 선택 기능이 포함된 입력 모달 구현.
* [ ] **데이터 그리드**: 국가/통화/계좌별 필터링 및 정렬이 가능한 고도화된 테이블.
* [x] **계좌번호 필드 추가**: 테이블, 입력 폼, CSV 가져오기/내보내기/샘플양식에 '계좌번호' 항목 추가 (6번째 순서).

### B. 시각화 및 분석 (Dashboard)

* [x] **자산 요약 카드**: 총 평가금액, 환차손익(예정), 포트폴리오 비중.
* [x] **뱃지(Badge) 시스템**:
  * 국가: 🇺🇸 USA (Blue), 🇰🇷 KOR (Red)
...
1. **Phase 1: 구조 설계**
    * [x] `AssetRecord` 타입 정의 및 로컬 스토리지(`asset_config_data`) 구성.
2. **Phase 2: UI 구현**
    * [x] 헤더 탭 확장 및 `AssetConfig` 메인 컴포넌트 개발.
    * [x] 테이블 및 입력 폼(Modal) UI 제작.
3. **Phase 3: 로직 연결**
    * [x] 매수/매도 금액 계산 로직 (환율 적용).
    * [x] CRUD 연동.
4. **Phase 4: 검증 및 최적화**
    * [x] 샘플 데이터 테스트 및 오차 검증.
